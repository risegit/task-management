...
const ManageDepartment = () => {
  ...
  // Parse filter parameters from location state - SIMPLIFIED
  useEffect(() => {
    if (location.state) {
  ...
  // Filter functions - SIMPLIFIED and made independent
  ...
  // Main data fetching effect - FIXED
  useEffect(() => {
  ...
  }, [userId, userCode]); // Only depend on userId and userCode

  // Filter tasks based on all filters - SIMPLIFIED
  const filteredTasks = useMemo(() => {
    if (!tasks.length) return [];
    
    return tasks.filter(task => {
      // Apply client filter
      if (selectedClient && selectedClient.value !== "all" && task.clientId !== selectedClient.value) {
        return false;
      }
      
      // Apply employee filter
      if (selectedEmployeeFilter && selectedEmployeeFilter.value !== "all") {
        if (!task.assignedToOriginal || 
  ...
  }, [tasks, selectedClient, selectedEmployeeFilter, selectedStatusFilter, selectedDeadlineFilter, searchQuery]);
