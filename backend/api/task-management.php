...
switch ($method) { 
    ...
    case 'GET':
        if($editTask){
            ...
            $sql1 = "SELECT t.id, t.client_id, t.task_name, c.name AS client_name, t.remarks, t.deadline, t.created_by, t.created_date, t.created_time, cb.name AS assigned_by_name, GROUP_CONCAT( DISTINCT ta.user_id ORDER BY ta.user_id SEPARATOR ', ' ) AS assigned_to_ids, GROUP_CONCAT( DISTINCT CONCAT(u.name, '||', d.color_code) ORDER BY u.name SEPARATOR ', ' ) AS assigned_to_names, ta.time, GROUP_CONCAT( DISTINCT u.department_id ORDER BY u.department_id SEPARATOR ', ' ) AS assigned_to_departments, $taskStatusCond AS task_status FROM tasks t INNER JOIN task_assignees ta ON t.id = ta.task_id INNER JOIN users u ON ta.user_id = u.id INNER JOIN users cb ON t.created_by = cb.id INNER JOIN clients c ON c.id = t.client_id LEFT JOIN departments d ON u.department_id = d.id $whereClause GROUP BY t.id ORDER BY t.id DESC";
            ...
            if (!empty($userCode)) {
                if (str_starts_with($userCode, 'ST')) {
                    ...
                    $deptData = $resultDept->fetch_assoc();
                    $deptId=$deptData['dept_id'];
                    $sql1 = "SELECT u.name, u.id AS user_id, t.client_id, t.task_name, t.priority, CASE WHEN t.deadline < CURDATE() AND ta.status IN ('not-acknowledge', 'acknowledge', 'in-progress') THEN 'overdue' WHEN ta.status IN ('not-acknowledge', 'acknowledge') THEN 'acknowledge' ELSE ta.status END AS status, GROUP_CONCAT(DISTINCT c.name ORDER BY c.name SEPARATOR ', ') AS clients, GROUP_CONCAT(DISTINCT t.task_name ORDER BY t.task_name SEPARATOR ', ') AS task_name, COUNT(t.id) AS count_tasks FROM tasks t INNER JOIN task_assignees ta ON t.id = ta.task_id INNER JOIN users u ON ta.user_id = u.id INNER JOIN clients c ON c.id = t.client_id WHERE ( ta.status IN ('not-acknowledge', 'acknowledge') OR ( ta.status IN ('in-progress', 'completed') AND ta.updated_date = CURDATE() ) ) AND ( u.department_id = '$deptId' OR ta.user_id = '$userId' OR EXISTS ( SELECT 1 FROM client_users cu WHERE cu.client_id = t.client_id AND cu.emp_id = '$userId' AND cu.is_poc = 1 ) ) GROUP BY u.id, CASE WHEN t.deadline < CURDATE() AND ta.status IN ('not-acknowledge', 'acknowledge', 'in-progress') THEN 'overdue' WHEN ta.status IN ('not-acknowledge', 'acknowledge') THEN 'acknowledge' ELSE ta.status END ORDER BY count_tasks DESC;";
                }
            }
