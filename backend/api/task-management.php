...
switch ($method) { 
    ...
    case 'GET':
        if($viewTask){
            $sql1 = "SELECT t.id, u1.name AS assignedby, u2.name AS assignedto, t.deadline, t.remarks, t.status FROM tasks t INNER JOIN users u1 ON t.assigned_by = u1.id INNER JOIN users u2 ON t.assigned_to = u2.id;";
